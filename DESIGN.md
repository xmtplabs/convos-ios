...
A dismissible inverted Bring your own agent callout uses a compact constellation of the provider marks already defined by the external-agent prototype. The same connection flow is reachable from More. Codex uses one-command Mac pairing and a revocable capability token; manual network fields stay behind an advanced disclosure. The persistent command bar names the active provider and offers a native switcher. Codex results render returned web links as explicit actions; the whole result or an individual link may be saved privately to Your Space and shared through the existing staged-composer boundary. Other providers remain labeled as demos, and “add this agent to a convo” is not presented as shipped behavior.
