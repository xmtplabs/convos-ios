...
public enum AgentBuilderTransition {
    public static let glassEffectId: String = "agentBuilderCard"
}

/// Model surface `AgentDraftComposer` renders. The app's
/// `AgentBuilderViewModel` conforms directly; the share extension provides a
/// slim local model whose Make stages a handoff instead of running the
/// generation (which outlives an extension's runway).
@MainActor
public protocol AgentDraftComposing: AnyObject, Observable {
    var composerTextBinding: Binding<String> { get }
    var pendingMediaAttachments: [PendingMediaAttachment] { get }
    var isRecordingVoiceMemo: Bool { get }
    var recordedVoiceMemo: (url: URL, duration: TimeInterval)? { get }
    var voiceMemoAudioLevels: [Float] { get }
    var voiceMemoRecorder: VoiceMemoRecorder { get }
    var isMakeEnabled: Bool { get }
    var isCommitting: Bool { get }
    /// False in hosts that can't record (the share extension has no
    /// microphone usage description); the voice button renders disabled.
    var supportsVoiceMemo: Bool { get }
    /// False in hosts whose staging path is photos-only.
    var allowsVideoAttachments: Bool { get }
...
/// A connection chip the host renders in the attachments row. The app maps
/// its `AgentBuilderConnection` cases here; hosts without connections pass
/// none and a nil `onConnectionsTap` (which hides the button).
public struct AgentDraftConnectionChip: Identifiable {
    public let id: String
    public let displayName: String
    public let chipImageName: String
    public let onRemove: () -> Void

    public init(id: String, displayName: String, chipImageName: String, onRemove: @escaping () -> Void) {
        self.id = id
        self.displayName = displayName
        self.chipImageName = chipImageName
        self.onRemove = onRemove
    }
}

public struct AgentDraftComposer<Model: AgentDraftComposing>: View {
    let viewModel: Model
    var focusState: FocusState<MessagesViewInputFocus?>.Binding
    /// Namespace owned by the host; the composer's outer glass rect applies
    /// `glassEffectID("agentBuilderCard", in:)` so it can morph into the
    /// in-stream summary cell on Make.
    let transitionNamespace: Namespace.ID
    let onMakeTap: () -> Void
    /// Connection chips shown in the attachments row (app-only concept).
    var connectionChips: [AgentDraftConnectionChip] = []
    /// Presents the host's connections sheet; nil hides the button.
    ...
    public init(
        ...
        onMakeTap: @escaping () -> Void,
        connectionChips: [AgentDraftConnectionChip] = [],
        onConnectionsTap: (() -> Void)? = nil
    ) {
        self.viewModel = viewModel
        self.focusState = focusState
        self.transitionNamespace = transitionNamespace
        self.onMakeTap = onMakeTap
        self.connectionChips = connectionChips
    ...
    public var body: some View {
        let photoMatching: PHPickerFilter = viewModel.allowsVideoAttachments
    ...
    private var alertsAttached: some View {
        card
    ...
    private var card: some View {
        let cardOpacity: Double = viewModel.isCommitting ? 0 : 1
        return Group {
    ...
    private var cameraPickerCover: some View {
        CameraPickerView(
            ...
            onVideoCaptured: viewModel.allowsVideoAttachments ? { url in
    ...
    private func handleSelectedPhotosChanged(to newValue: [PhotosPickerItem]) {
        ...
        Task {
            for item in items {
                if viewModel.allowsVideoAttachments,
    ...
    private var attachmentsRow: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: DesignConstants.Spacing.step2x) {
                ...
                ForEach(connectionChips.sorted { $0.id < $1.id }) { chip in
                    ConnectionAttachmentChip(chip: chip)
                }
            }
    ...
    private var scrollableMediaButtons: some View {
        ...
        return ScrollView(.horizontal, showsIndicators: false) {
            MessagesMediaButtonsView(
                ...
                isVoiceMemoDisabled: viewModel.recordedVoiceMemo != nil || !viewModel.supportsVoiceMemo,
                showsFileButton: false,
                buttonSpacing: DesignConstants.Spacing.step4x,
                onConnectionsTap: onConnectionsTap.map { tap in
                    {
                        focusState.wrappedValue = nil
                        tap()
                    }
...
private struct ConnectionAttachmentChip: View {
    let chip: AgentDraftConnectionChip
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            Image(chip.chipImageName)
                .resizable()
                .scaledToFit()
                .frame(width: chipSize, height: chipSize)
                .accessibilityLabel("\(chip.displayName) connection attachment")
                .accessibilityIdentifier("connection-attachment-\(chip.id)")
            ...
            .padding(.top, DesignConstants.Spacing.step2x)
            .padding(.trailing, DesignConstants.Spacing.step2x)
            .accessibilityLabel("Remove \(chip.displayName) connection")
            .accessibilityIdentifier("remove-connection-\(chip.id)-button")
        }
        .scaleEffect(isPoofing ? 1.3 : 1.0)
        .blur(radius: isPoofing ? 12.0 : 0.0)
        .opacity(isPoofing ? 0.0 : 1.0)
