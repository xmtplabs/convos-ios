    ...
    func setup(item: MessagesListItemType, config: CellConfig) {
        ...
        contentConfiguration = UIHostingConfiguration {
            Group {
                ...
                case .conversationInfo(let conversation):
                    VStack(spacing: DesignConstants.Spacing.step4x) {
                        ConversationInfoPreview(
                            ...
                            infoSheetContent: config.backwardsSecrecyInfoSheet
                        )
                    }
                    .padding(.vertical, DesignConstants.Spacing.step4x)
                    .padding(.horizontal, DesignConstants.Spacing.step4x)
                ...
                case let .agentDmInfo(agentProfile, agentName):
                    AgentDmInfoCellView(agentProfile: agentProfile, agentName: agentName)
