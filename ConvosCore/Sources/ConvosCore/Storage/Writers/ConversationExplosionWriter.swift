...
struct XMTPExplodeGroupOperations: ExplodeGroupOperationsProtocol {
    ...
    private func findGroupConversation(
        ...
        let inboxReady = try await sessionStateManager.waitForInboxReadyResult()
        // Drop into the XMTPiOS layer for the explode/leave operations —
        // those still call native Group methods (`sendExplode`, `leaveGroup`)
        // that haven't been hoisted onto `MessagingGroup` yet.
        guard let messagingConversation = try await inboxReady.client.messagingConversation(
            with: conversationId
        ),
              let xmtpConversation = messagingConversation.underlyingXMTPiOSConversation else {
