...
class MessagesRepository: MessagesRepositoryProtocol {
    ...
    func fetchInitial() throws -> [AnyMessage] {
        ...
        Log.info("[perf] MessagesRepository.fetchInitial: \(String(format: "%.1f", fetchMs))ms (\(result.count) messages, pageSize=\(pageSize))")
        return result
    }
...
fileprivate extension Database {
    func composeMessages(
        ...
        let convMs = (CFAbsoluteTimeGetCurrent() - convStart) * 1000

        let membersStart = CFAbsoluteTimeGetCurrent()
        ...
        let membersMs = (CFAbsoluteTimeGetCurrent() - membersStart) * 1000
        ...
        let msgMs = (CFAbsoluteTimeGetCurrent() - msgStart) * 1000
        ...
        let composeMs = (CFAbsoluteTimeGetCurrent() - composeStart) * 1000

        let totalMs = (CFAbsoluteTimeGetCurrent() - totalStart) * 1000
        // swiftlint:disable:next line_length
        Log.info("[perf] composeMessages: total=\(String(format: "%.1f", totalMs))ms conv=\(String(format: "%.1f", convMs))ms members=\(String(format: "%.1f", membersMs))ms msgQuery=\(String(format: "%.1f", msgMs))ms compose=\(String(format: "%.1f", composeMs))ms rows=\(rawMessages.count) msgs=\(result.0.count)")
        return result
    }
}
