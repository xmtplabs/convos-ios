...
public struct InboxReadyResult: @unchecked Sendable {
    ...
    /// - MessagingClient wraps XMTPiOS.Client which is not Sendable
...
public actor SessionStateMachine: SessionStateManagerProtocol {
    ...
    /// with `MessagingClient` / `InboxReadyResult` wrap thread-safe XMTP
    /// types designed for async/await use.
    enum Action: @unchecked Sendable {
        case authorize(inboxId: String),
             register,
             clientAuthorized(any MessagingClient),
             clientRegistered(any MessagingClient),
    ...
    private func handleAuthorize(inboxId: String) async throws {
        ...
        let client: any MessagingClient
        do {
            try Task.checkCancellation()
            client = try await buildXmtpClient(
                inboxId: identity.inboxId,
                identity: keys.privateKey.identity,
    ...
    private func handleClientAuthorized(client: any MessagingClient) async throws {
        try Task.checkCancellation()
    ...
    private func handleClientRegistered(client: any MessagingClient) async throws {
        try Task.checkCancellation()
    ...
    private func handleDelete() async throws {
        ...
        let liveContext: (client: any MessagingClient, apiClient: any ConvosAPIClientProtocol)?
        switch _state {
    ...
    private func createXmtpClient(signingKey: SigningKey,
                                  options: ClientOptions) async throws -> any MessagingClient {
