...
public protocol SessionManagerProtocol: AnyObject, Sendable {
    ...
    /// Cache-miss fallback for the default-agent flow: a conversation the state
    /// machine created fresh (no warm-cache claim, so `commitClaimedConversation`
    /// never fires) is already visible and in use. Ensures the default agent is
    /// provisioned into it and sends the one-shot `conversation_ready` greeting
    /// cue. Best-effort and idempotent.
