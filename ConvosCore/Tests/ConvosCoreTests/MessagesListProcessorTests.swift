...
struct MessagesListProcessorEdgeCaseTests {
    ...
    @Test("processWithPagination delegates to process")
    func processWithPaginationDelegates() {
        let now = Date()
        let messages = [
            makeMessage(id: "1", sender: otherUser, text: "Hello", date: now),
        ]
        let resultDirect = MessagesListProcessor.process(messages)
        let resultPaginated = MessagesListProcessor.processWithPagination(messages, isLoadingPrevious: true)
        #expect(resultDirect.count == resultPaginated.count)
    }
