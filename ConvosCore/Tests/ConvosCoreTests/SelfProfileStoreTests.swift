...
struct SelfProfileStoreTests {
    ...
    func grdbContract() async throws {
        ...
        let store = GRDBSelfProfileStore(databaseWriter: queue, databaseReader: queue)
    ...
    private func runContract(_ store: any SelfProfileStoreProtocol) async throws {
        ...
        try await store.save(DBSelfProfile(inboxId: "me", name: "Me", updatedAt: t))
        let loaded = try await store.load()
        #expect(loaded?.name == "Me")

        try await store.save(DBSelfProfile(inboxId: "me", name: "Me Renamed", updatedAt: t))
