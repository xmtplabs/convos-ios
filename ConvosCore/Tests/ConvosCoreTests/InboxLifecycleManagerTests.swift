...
struct InboxLifecycleManagerTests {
    ...
    func testRebalanceSleepsExcessInboxes() async throws {
        ...
        // Rebalance
        await manager.rebalance(activeClientId: nil)
    ...
    func testRebalanceSleepsNonTopNInboxes() async throws {
        ...
        // Rebalance with client-3 as active - client-2 should potentially be slept if over capacity
        await manager.rebalance(activeClientId: "client-3")
    ...
    func testRebalanceProtectsActiveClient() async throws {
        ...
        // Now rebalance with client-1 (oldest activity) as the active client
        await manager.rebalance(activeClientId: "client-1")
    ...
    func testRebalanceSleepsOldInboxWhenSwitchingConversations() async throws {
        ...
        // Rebalance with client-new as active (user switched away from client-old)
        await manager.rebalance(activeClientId: "client-new")
    ...
    func testRebalanceWithNoActiveClient() async throws {
        ...
        // Rebalance with no active client (user navigated to conversation list)
        await manager.rebalance(activeClientId: nil)
    ...
    func testRebalanceWakesMoreRecentInboxWhenNavigatingBack() async throws {
        ...
        // Rebalance with inbox 2 as active - inbox 1 should be slept (over capacity)
        await manager.rebalance(activeClientId: "client-2")
