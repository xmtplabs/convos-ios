...
| Identity share DM dropped by network | A's `sharingIdentity` has no timer (the user already confirmed). On send failure we transition to `.failed` immediately; the user dismisses and re-pairs. We deliberately don't auto-retry — identity share is the last step before adoption, on a critical path where a transparent retry could mask real backend errors, and the handshake is short enough that asking the user to retry is acceptable. |
