...
**Send-tool contract (Tier 2 — proactive cross-member outreach).** Proactive outreach into another member's lane is a dedicated tool, `convos_message_member(member, message)`. The agent names the member (display name or `dm:` label); Hermes resolves it against the **injected roster** (which contains only live, non-revoked lanes), so an unknown/stale member is rejected before any send — the roster is the guardrail by construction. Because display names are non-unique (§5.2, line 179 — names change and collide; the `dm:` label is keyed on inbox id precisely to avoid this), a name that matches two active lanes is **rejected as ambiguous**: the tool refuses it and the agent must re-address by the unique `dm:` label, so a proactive private message can never land on the wrong same-named member. The tool sends straight into that member's lane; the agent's own plain-text reply this turn still routes to the person it is answering. That separation is the whole point: outreach is a structured side effect, the reply is the acknowledgement, and the two can never share a lane. Read receipts are automatic per channel, never model-controlled. Herald's own membership enforcement backstops the tiny mid-turn-revocation window.

> Evolution: an earlier design routed outreach via a `CHANNEL:dm:<id>` text marker parsed from the reply, redirecting the whole reply to one lane. Two-simulator QA showed the model could not reliably self-partition a single turn across two destinations as free text — the outreach leaked into the asker's lane, and after prompt hardening the acknowledgement leaked into the peer's lane. The marker was replaced by the `convos_message_member` tool, which removes the model's ability to mis-route by making the destination a structured argument. The marker parser is retained as a harmless fail-closed path but is no longer taught to the model.
...
- **Layer 2 — reconciliation sweep (bounded staleness):** a scheduled job (piggyback the DO alarm cadence or worker cron, e.g. hourly) fetches current primary members and, for any registry-DM whose peer is not in the set, performs the **same immediate cutoff as Layer 1** — registry status → `revoked` (DO refuses all sends), Herald **detaches** the conversation, and the agent calls `requestRemoval()` on the DM group. It must not merely `leave` the MLS group: `leave`/`requestRemoval` is peer-committed and does not stop delivery while pending (§5.5.1), so a missed-webhook peer would keep reaching the agent until their client commits. The revoke + detach + DO refusal are the guaranteed cutoff; the leave is the visible ending on top. Catches missed webhooks, Herald restarts mid-stream, and any ordering races. Idempotent and cheap (one members call + registry diff).
...
2. **Policy module**: handle `conversation_added` webhooks — require exactly 2 members and peer ∈ primary members (Herald members list), then **atomically reserve the peer's slot** (a single synchronous conditional INSERT of a `pending` row) *before* the async attach, per §5.2.1. The reservation — not a check-then-attach — is what rejects a concurrent second DM for the same peer; the loser of the race leaves its group. On attach success the row goes `pending→active`; on failure (or a lost reservation) the group is left and the pending row deleted. Idempotent per conversation and per peer. All decisions logged/metric'd.
