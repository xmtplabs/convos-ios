...
- **Phase 0 — protocol validation spike** (throwaway code, local stack): S1 confirm Herald can observe a welcome for a 2-member group it was never asked to attach (conversation stream) and measure add→observation latency; S2 confirm attach-then-stream works for such a group end to end (profile publish renders + verifies on iOS, agent replies land); S3 exercise the membership edges — third member added (leave fires), peer removed from primary (leave fires), agent removed from the DM (channel closes, instance survives); S4 confirm `ConversationCustomMetadata` classification syncs to a second device of the same user; S5 confirm fleet convergence — a rebooted container of a pre-deploy instance boots the *new* Hermes image (check the container lifecycle ADR + wrangler config; prod agents must not be version-pinned the way dev `AgentVariant` runtimes are). Exit criteria: all five demonstrably true, add→first-reply latency acceptable (< ~10s including container wake). *This closes CON-761 with a viability verdict and hard numbers.*
...
- **Removed-member lockout** is enforced by MLS membership (the agent leaves; two layers, §5.5) — not by prompting. Verify in QA: removed member's DM group has no agent to deliver to, and the DO refuses sends to revoked registry entries.
