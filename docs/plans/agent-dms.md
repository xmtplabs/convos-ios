...
**Send-tool contract (Tier 2 — proactive cross-member outreach).** Proactive outreach into another member's lane is a dedicated tool, `convos_message_member(member, message)`. The agent names the member (display name or `dm:` label); Hermes resolves it against the **injected roster** (which contains only live, non-revoked lanes), so an unknown/stale member is rejected before any send — the roster is the guardrail by construction. The tool sends straight into that member's lane; the agent's own plain-text reply this turn still routes to the person it is answering. That separation is the whole point: outreach is a structured side effect, the reply is the acknowledgement, and the two can never share a lane. Read receipts are automatic per channel, never model-controlled. Herald's own membership enforcement backstops the tiny mid-turn-revocation window.
