...
| | |
|:---:|:---:|
| <img src="agent-template-mocks/01-agent-contact-page.svg" width="260" alt="Agent template contact page"/><br/>**1. Agent template contact page** — hero surface. In **Phase 1, this is accessible only from inside a chat** (tap the agent's avatar in a message bubble, the member list, the read-receipts row). The chat plus-menu's "Contact info" path on a verified-agent member resolves here. Avatar, name, description, creator attribution, agent badge, **Chat** (primary CTA — Phase-1 default spawns a fresh instance in a new convo; the existing-1:1 short-circuit is a fast-follow inside Phase 1), **Share** (offers the iOS share sheet with `convos://template/<id>` + the Universal Link), About section, Skills chips, **Convos with you** list, destructive footer. No Edit, no Duplicate. | <img src="agent-template-mocks/05-deeplink-card-in-chat.svg" width="260" alt="Deeplink card rendered as a chat bubble"/><br/>**2. Deeplink card rendered in a chat** — when a `convos://template/<id>` URL lands in a Convos message bubble, the iOS client recognises the URL pattern and renders the link inline as a clean card (avatar + name + description + "Convos · agent template" footer + Open ›). Tapping the card opens a new conversation with a fresh instance of that template — same end-state as tapping the Universal Link outside the app. Replaces the raw URL inline so the chat doesn't show an ugly UUID string. |
| <img src="agent-template-mocks/04-web-preview.svg" width="260" alt="convos.org/template/&lt;id&gt; web preview"/><br/>**3. Web preview at `convos.org/template/<id>`** — install-prompt page that renders when a non-Convos user follows the Universal Link. Modeled on today's `agents-dev.convos.org` "Mise" surface. Browser frame, Convos header, "Install Convos, then scan the QR to chat" banner with App Store + Google Play badges, QR encoding the `convos://template/<id>` deeplink, agent name + description + skill chips. On a device with the app installed, the Universal Link is intercepted before the web page renders. | |
...
3. **P1.3 — Share button — generates the deeplink.** Tapping **Share** on the contact view opens the iOS share sheet with `convos://template/<id>` (prod) / `convos-dev://template/<id>` (dev) plus the matching Universal Link `https://convos.org/template/<id>` (or `https://agents-dev.convos.org/template/<id>` in dev). The Universal Link is what gets shared — iOS handles the route-to-app-or-web distinction at tap time.
4. **P1.4 — Deeplink rendered in a chat as a card.** When a `convos://template/<id>` or `https://convos.org/template/<id>` URL appears in a Convos message body, the iOS client recognises the URL pattern in the message-rendering pipeline and replaces the inline URL with a clean preview card (surface 2 in §2). The card shows the agent's avatar, name, description, a "Convos · agent template" footer, and an "Open ›" affordance. Tap → routes to surface 1 (the contact view) for the local user. The card hydrates by calling `GET /api/v2/agent-templates/<id>` against the existing CRUD; standard 404 + caching applies.
5. **P1.5 — Deeplink tapped outside Convos — Universal Link routing.** On a device with Convos installed, tapping the Universal Link in any app (Messages, Mail, Safari, Slack, anywhere) routes directly into a new conversation with a fresh instance of the template — same end-state as P1.2's Chat-button path. On a device *without* Convos, the URL renders the web preview at `convos.org/template/<id>` (surface 3 in §2) with App Store / Google Play badges and a QR that encodes the `convos://template/<id>` form for cross-device pairing. The web-preview HTML is hosted by the convos.org web frontend (not `convos-backend`); the page consumes the backend's existing public detail endpoint `GET /api/v2/agent-templates/:idOrHashedSlug` to render template metadata. See FAQ #3.
...
- **The recipe-sharer (Phase 1)**: Cameron builds a "Pro Cycling Coach" agent and wants to share it with three friends on a group ride. He hits Share on the agent contact view, drops the Universal Link into their group chat, the link renders as the deeplink card, and each friend taps the card once to get their own private 1:1 with a fresh instance of the agent. No contacts list involved — the chat *is* the discovery surface.
...
- **The non-Convos friend (Phase 1)**: Bob doesn't have Convos installed. He taps the same Universal Link in iMessage; iOS resolves it to the `convos.org/template/<id>` web preview, he sees what the agent does, hits the App Store badge, installs Convos, and on first launch the deferred-deeplink flow lands him in a fresh conversation with the agent.
...
- [ ] **(Phase 1)** **Deeplink-card tap routing.** Does tapping the in-chat deeplink card (surface 2) route to the contact view (surface 1) or directly into a new conversation with a fresh instance? Phase 1 default per §3 item P1.4 is "route to the contact view, user taps Chat to spawn." A one-tap "spawn on card tap" alternative is faster but harder to undo. Designer + product call.
