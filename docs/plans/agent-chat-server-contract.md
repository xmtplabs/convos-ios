...
3. Add server-side adapters for the Hermes OpenAI-compatible API server and the OpenClaw Gateway WebSocket protocol. Treat Grok Bot as a separate future connector and do not substitute the Grok API for a provider-supported Grok Bot integration.
...
Codex/Claude Code pairing requires a desktop bridge session, provider-auth status, approved workspace identifiers, and a bridge public key. Hermes requires an HTTPS-reachable gateway URL plus an `API_SERVER_KEY`. OpenClaw requires a secure WebSocket gateway URL, device identity/signature, gateway token, and the minimum operator scopes. Grok Bot requires a documented provider-supported connector before it can leave prototype-only status; Convos must not request or store a Grok/xAI key as a substitute.
