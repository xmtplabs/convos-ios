...
///
/// Exercises `SessionManager.deleteExpiredPendingInvites()` over a
/// `MockDatabaseManager` + `MockKeychainIdentityStore`. No
/// `MessagingClient` is ever instantiated — pure DB + identity-store
/// unit coverage. No `DualBackendTestFixtures` needed; backend-agnostic
/// and identical under DTU and xmtpiOS.
///
/// One adjustment from the source: added `conversationEmoji: nil` and
/// `hasHadVerifiedAssistant: false` to the `DBConversation`
/// initializer to match the current schema.
