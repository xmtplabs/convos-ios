...
/// `ConvosCore/Tests/ConvosCoreTests/TypingIndicatorManagerTests.swift`.
///
/// Pure-unit test. `TypingIndicatorManager` + `TypingIndicatorCodec` are
/// both fully public in ConvosCore and don't touch a `MessagingClient`,
/// so the migration is mechanical — re-host the file verbatim.
///
/// The DTU backend supports the `typing_indicator` content type in the
/// `supported_exact` set, so exercising the codec + manager in
/// `ConvosCoreDTUTests` proves the pair work unchanged under the DTU
/// target.
///
/// No `DualBackendTestFixtures` needed: there's no backend-crossing
/// behavior here. The codec is a JSON encode/decode; the manager is a
/// thread-local dictionary. Both are identical on both backends.
