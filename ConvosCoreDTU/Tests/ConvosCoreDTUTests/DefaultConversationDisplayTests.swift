...
///
/// Pure-unit test — exercises `EmojiSelector`, `[Profile]`
/// `formattedNamesString` / helpers, and `Conversation.avatarType` /
/// `computedDisplayName` / `defaultEmoji` / `isFullyAnonymous`. None of
/// those surfaces touch a `MessagingClient` or database. The migration
/// is a verbatim re-host into the ConvosCoreDTU test target. No fixture
/// needed.
...
struct DefaultConversationDisplayTests {
    ...
    func avatarTypeProfileForDMWithConversationEmoji() {
        ...
        if case .profile(let profile, _) = conversation.avatarType {
            // `ConversationMember.mock(isCurrentUser: false, ...)` stamps
            // the profile inboxId as `other-user-<UUID>` so we match on
            // the prefix rather than an exact literal.
            #expect(profile.inboxId.hasPrefix("other-user-"))
