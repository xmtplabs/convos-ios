...
/// `ConvosCore/Tests/ConvosCoreTests/ChronologicalSortIdTests.swift`.
///
/// Pure-DB test — `IncomingMessageWriter.chronologicalSortId(...)` is a
/// GRDB query that doesn't reach for a `MessagingClient`. The test was
/// running against `MockDatabaseManager.makeTestDatabase()` directly in
/// ConvosCore's suite; it re-hosts verbatim into ConvosCoreDTU's test
/// target where `@testable import ConvosCore` keeps
/// `IncomingMessageWriter.chronologicalSortId` (internal static) reachable.
///
/// No `DualBackendTestFixtures` needed — there's no backend-crossing
/// behavior to prove. The database layer is identical for both backends.
