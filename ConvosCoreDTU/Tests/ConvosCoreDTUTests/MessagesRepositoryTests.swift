...
/// `ConvosCore/Tests/ConvosCoreTests/MessagesRepositoryTests.swift`.
///
/// Pure-DB coverage of `MessagesRepository.fetchInitial()` — verifies
/// that messages, reactions, and replies authored by removed members
/// remain visible (using the "Removed" / "Somebody" fallback profiles)
/// and that historical `DBMemberProfile` rows survive sync upserts.
///
/// No `MessagingClient` interaction at all — the test seeds GRDB rows
/// directly and reads through `MessagesRepository`. Migration is a
/// verbatim re-host onto the DTU lane so both backends execute the
