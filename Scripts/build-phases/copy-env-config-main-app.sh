...
# Part 1c: Generate Secrets.swift for Prod builds (the "Convos (Prod)" scheme
# uses the "Release" configuration). Local Prod builds skip `make secrets`, so
# without this branch a stale Local-build Secrets.swift (with an auto-detected
# LAN/Tailscale IP) would leak into the Prod app. We deliberately force the
# backend URL and XMTP host EMPTY so ConfigManager falls back to the prod
# source-of-truth in config.prod.json (api.prod.convos.xyz, production XMTP
# network) — never an .env override, which could point at a local backend.
# Mirrors the CI path in generate-secrets-secure.sh, which also emits empties.
elif [ "$TARGET_NAME" = "Convos" ] && [ "$CONFIGURATION" = "Release" ]; then
    ...
    # PostHog/Sentry come from .env only when present (empty in CI). Firebase
    # debug token stays empty in Prod (production App Check uses real attestation).
    POSTHOG_API_KEY=""
    SENTRY_DSN=""
    if [ -f "${SRCROOT}/.env" ]; then
        POSTHOG_API_KEY=$(grep -v '^#' "${SRCROOT}/.env" | grep '^POSTHOG_API_KEY=' | cut -d'=' -f2- | sed -e 's/^"//' -e 's/"$//' || true)
    ...
    if [ -n "$SENTRY_DSN" ]; then
        echo "✅ Found SENTRY_DSN in .env (Sentry enabled for this Prod build)"
    fi

    # Generate Secrets.swift for Prod
    cat > "$SECRETS_FILE" << EOF
import Foundation
...
/// Secrets are generated automatically for Prod (Release) builds
enum Secrets {
