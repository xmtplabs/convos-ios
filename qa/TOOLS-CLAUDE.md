...
- **`idb` resolution.** Resolve from PATH (`IDB=$(command -v idb)`) once at session start. The `fb-idb` client (1.1.7) crashes under Python >= 3.14 (`asyncio.get_event_loop` was removed); install it into a Python 3.13 venv (`python3.13 -m venv <dir> && <dir>/bin/pip install fb-idb`) and put that venv's `bin` on PATH. `idb-companion` comes from Homebrew (`brew tap facebook/fb && brew install idb-companion`).
