...
3. Verify the sheet shows:
   - A name field (`profile-setup-name-field`) with an avatar on the left (`profile-setup-avatar`) and photo/camera buttons on the right (`profile-setup-photo-button`, `profile-setup-camera-button`).
   - The "I agree to Convos Privacy & Terms" row with a toggle (`profile-setup-terms-toggle`).
   - A "Come in" button (`profile-setup-save-button`), disabled while the name field is empty or the terms toggle is off.

### Complete profile setup

4. Enter a display name like "QA Tester" in `profile-setup-name-field`. The avatar updates to a monogram of the typed name.
