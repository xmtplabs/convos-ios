...
19. Read the pairing URL from the app event log using `sim_log_events(udid=DEVICE_A_UDID, event_filter="vault.pairing_url_created")`. The URL is in the `url=` parameter of the event line. The app emits a `vault.pairing_url_created` event when the invite is generated.
20. Verify the URL has the format `https://dev.convos.org/pair/<slug>?expires=<unix_timestamp>`.
...
23. A 6-digit pin should be displayed (`pairing-pin-display`). Read the pin text from the accessibility tree.
24. An expiry countdown should be visible.
25. Text should indicate "the other device is requesting to pair" and instruct to enter the code on the other device.

### Enter pin on Device A

26. On **Device A**, the pairing sheet should have transitioned from the QR code state to the pin entry state. Wait for the pin entry field (`pin-entry-field`) to appear (this may take several seconds as the join request travels over the network).
27. The sheet should show the name of Device B and "Enter the code shown on the new device".
28. Type the 6-digit pin (read from step 23) into the pin entry field.
29. The "Approve" button (`approve-button`) should become enabled once all 6 digits are entered.

### Approve pairing

30. Tap the "Approve" button on **Device A**.
31. The sheet should transition to the "syncing" state showing "Pairing..." with a rotating sync icon.
32. After a few seconds, the sheet should transition to the "completed" state. The title should change to "Device added" and show a checkmark icon with Device B's name.
33. Tap "Got it" (`got-it-button`) to dismiss the pairing sheet.

### Verify Device A shows both devices

34. The Devices screen should now show two devices in the list.
35. One device should have the "This device" label (Device A).
36. The other device should show Device B's simulator name.

### Verify conversation sync on Device A

37. Navigate back to the conversations list on **Device A** (tap the back button from Devices → Settings, then dismiss settings).
38. The conversations list should contain **both** "Device A Convo" and "Device B Convo". Device B's conversation was synced to Device A as part of the pairing key share.

### Verify conversation sync on Device B

39. On **Device B**, dismiss the pairing sheet if it's still showing (it may have transitioned to completed, or may still be on the pin screen — either dismiss or tap "Got it" if available).
40. Navigate to the conversations list on **Device B**.
41. The conversations list should contain **both** "Device A Convo" and "Device B Convo". Device A's conversation was synced to Device B as part of the pairing key share.
