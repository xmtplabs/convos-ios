...
- Voice transcription and imported files stay on-device. Built-in chat answers remain local and use the currently visible briefing data; when the user selects connected Codex, Town, or Tasklet, the app sends the request and only the explicitly enabled, bounded Your Space snapshot to that connected agent.
