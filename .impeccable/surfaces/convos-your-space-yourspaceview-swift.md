...
- Constraints: private by default; never auto-send or imply server-side synthesis; a connected Codex receives only the bounded snapshot the user enables and works inside the paired Mac workspace; preserve existing start, join, pairing, deep-link, stale-device, settings, and conversation flows.
- Direction: a quiet personal signal desk using Convos neutrals, dramatic editorial type, one inverted attention surface, a dismissible inverted bring-your-own-agent callout, and a visible bottom agent command bar between native More and chat controls. The home distills Useful details to compact typed filter rows; All Context gives automatically indexed facts full-width cards with the complete source message, sender, convo, time, source navigation, sender DM, and explicit sharing. The optional agent widget links agent identity to its convo and private lane.
