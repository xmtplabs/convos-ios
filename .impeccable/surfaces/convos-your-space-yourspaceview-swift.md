...
- Job and action: orient from one private narrative, open or share a useful thing that changed, return to the source Convo, inspect every agent the person can use and its access, or search context across their life.
...
- Direction: a quiet personal signal desk using Convos neutrals and dramatic editorial type. Reading order is narrative → documents and artifacts updated across Convos → a mixed list of seven recent Convos and agent updates → ownership-aware Agents → All my things → optional support. The bottom agent command bar remains between native More and chat controls.
- Navigation: the profile avatar pushes All my things, whose own gear opens current settings. Updated things open a preview with source-Convo and Share anywhere actions. Recent Convos push a full-height transcript focus; recent agent updates open that agent's private lane. Each agent profile is the app-level control center for Use anywhere, connections, and group-specific permissions.
...
- Memorable moment: the launch sentence immediately resolves into a useful document with two obvious actions—open its Convo or share it anywhere—then the agent page makes continuous access explicit with Listen, Talk, and Pause.
