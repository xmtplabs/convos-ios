...
- Scope and mode: the authenticated iOS landing surface; Operate mode distilled to one `@doc` product rather than an inbox or dashboard.
- Audience: a person with something messy from a group—screenshots, links, messages, files, or voice—who wants one useful thing to share back.
- Job and action: name what the group should keep track of, give `@doc` source material, make one living doc, then reopen or share it.
...
- Proof and content: the maker accepts a plain-language intent or explicit attachment; the library uses real content-first photo/file/link/map previews, source Convo names, verified sender metadata, and relative time. The production document-generation service remains an open data boundary.
...
- Direction: one drenched Lava maker with rounded black display type, a white input well, a black action, and flat document rows on the native canvas. Reading order is maker → Your docs. There is no bottom dock.
- Navigation: the profile avatar pushes All my things. The top-right native overflow keeps New doc, Your docs, All convos, All my things, start/join, and settings available without putting them into the Home story. Doc rows open a preview, source Convo, or Share anywhere flow.
...
- Memorable moment: “Turn anything into a group doc” resolves immediately into a staged `@doc` request, then the resulting artifact offers Save doc and Share anywhere without requiring an engine connection.
