...
struct PairingSheetView: View {
    ...
    private func qrCodeContent(url: String) -> some View {
        ...
        VStack(spacing: DesignConstants.Spacing.step4x) {
            ...
            let qrURL = URL(string: url)
            if let qrURL, !url.isEmpty {
                QRCodeView(
                    url: qrURL,
...
#Preview("QR Code - Blurred") {
    ...
    .selfSizingSheet(isPresented: $isPresented) {
        let vm = PairingSheetViewModel(pairingService: MockPairingService(), appGroupIdentifier: nil)
...
#Preview("Emoji Confirmation") {
    PairingSheetPreview(
        state: .emojiConfirmation(emojis: ["🦊", "🎸", "🌊"], deviceName: "Jarod's iPad"),
...
private struct PairingSheetPreview: View {
    ...
    var body: some View {
        let vm = PairingSheetViewModel(pairingService: MockPairingService())
        PairingSheetView(viewModel: vm)
            .onAppear {
                vm.flowState = state
