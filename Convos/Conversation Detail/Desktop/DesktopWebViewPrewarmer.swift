...
///
/// Only runs when desktop mode is enabled, and only after the session reaches
/// inbox-ready, so the warm-up stays off the contention-heavy launch path
/// (mirrors `ConvosApp.startMetricsIdentification`). Everything that can run
/// off the main thread does; only the UIKit/WebKit touches hop to the main
/// actor.
enum DesktopWebViewPrewarmer {
    ...
    static func prewarmIfNeeded(session: any SessionManagerProtocol) {
        Task.detached(priority: .utility) {
            guard await MainActor.run(body: { FeatureFlags.shared.isDesktopModeEnabled }) else { return }
            do {
