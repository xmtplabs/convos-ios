...
// The scrollable desktop surface rendered behind the conversation sheet on
// the Desktop tab: the web view as the hero card, then an optional invite
// card (the same `InviteCodeBody` the transcript's index-0 invite cell and
// `InviteCodeOverlay` compose).
//
// The `ConversationBottomSheet` floats above this view; its compact resting
// height is shared via `ConversationSheetMetrics.compactRestingHeight`. The
// web section is sized so it fills the space above the resting sheet, and
// the scroll content reserves a matching bottom margin so the last section
...
struct DesktopLayoutView: View {
    ...
    /// The sheet's live occupied height (keyboard included). The scroll content
    /// insets by it so the last section always clears whatever the sheet
    /// currently covers. Defaults to the compact resting height for the first
    /// frame before the sheet reports.
    ...
    var body: some View {
        GeometryReader { proxy in
            let availableWebHeight: CGFloat = proxy.size.height - ConversationSheetMetrics.compactRestingHeight - Constant.sectionSpacing
            let webSectionHeight: CGFloat = max(availableWebHeight, Constant.minimumWebSectionHeight)
            let bottomInset: CGFloat = sheetHeight
            ScrollView {
