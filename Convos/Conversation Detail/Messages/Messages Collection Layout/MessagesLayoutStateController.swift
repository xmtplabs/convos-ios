...
final class MessagesLayoutStateController<Layout: MessagesLayoutProtocol> {
    ...
    struct AdditionalLayoutAttributes {
        ...
        fileprivate init(_ layoutRepresentation: MessagesLayoutProtocol) {
    ...
    func contentHeight(at state: MessagesCollectionLayoutModelState) -> CGFloat {
    ...
    func layoutAttributesForElements(in rect: CGRect,
                                     state: MessagesCollectionLayoutModelState,
                                     ignoreCache: Bool = false) -> [MessagesLayoutAttributes] {
        let predicate: (MessagesLayoutAttributes) -> ComparisonResult = { attributes in
        ...
        if !ignoreCache,
           ...
           cachedAttributesState.rect.contains(rect) {
            ...
            let visibleAttributes: [MessagesLayoutAttributes]
            if rect == totalRect {
    ...
    func resetCachedAttributeObjects() {
        for state in MessagesCollectionLayoutModelState.allCases {
            resetCachedAttributeObjects(at: state)
        }
    }

    private func resetCachedAttributeObjects(at state: MessagesCollectionLayoutModelState) {
    ...
    func itemAttributes(for itemPath: ItemPath,
                        ...
                        at state: MessagesCollectionLayoutModelState,
                        additionalAttributes: AdditionalLayoutAttributes? = nil) -> MessagesLayoutAttributes? {
        let additionalAttributes = additionalAttributes ?? AdditionalLayoutAttributes(layoutRepresentation)

        let attributes: MessagesLayoutAttributes
        ...
        case .cell:
            ...
            if let cachedAttributes = cachedAttributeObjects[state]?[.cell]?[itemPath] {
                ...
                attributes = MessagesLayoutAttributes(forCellWith: itemIndexPath)
    ...
    func itemPath(by itemId: UUID, kind: ItemKind, at state: MessagesCollectionLayoutModelState) -> ItemPath? {
        layout(at: state).itemPath(by: itemId, kind: kind)
    }

    func sectionIdentifier(for index: Int, at state: MessagesCollectionLayoutModelState) -> UUID? {
        let layout = layout(at: state)
    ...
    func sectionIndex(for sectionIdentifier: UUID, at state: MessagesCollectionLayoutModelState) -> Int? {
    ...
    func itemIdentifier(for itemPath: ItemPath, kind: ItemKind, at state: MessagesCollectionLayoutModelState) -> UUID? {
        let layout = layout(at: state)
        guard itemPath.section < layout.sections.count else {
            // This occurs when getting layout attributes for initial / final animations
            return nil
    ...
    func numberOfSections(at state: MessagesCollectionLayoutModelState) -> Int {
        layout(at: state).sections.count
    }

    func numberOfItems(in sectionIndex: Int, at state: MessagesCollectionLayoutModelState) -> Int {
        layout(at: state).sections[sectionIndex].items.count
    }

    func item(for itemPath: ItemPath, kind: ItemKind, at state: MessagesCollectionLayoutModelState) -> ItemModel? {
        let layout = layout(at: state)
        switch kind {
        case .header:
            guard itemPath.section < layout.sections.count,
    ...
    func update(preferredSize: CGSize,
                alignment: MessagesListItemAlignment,
    ...
    func contentSize(for state: MessagesCollectionLayoutModelState) -> CGSize {
    ...
    func layout(at state: MessagesCollectionLayoutModelState) -> LayoutModel<Layout> {
        switch state {
        case .beforeUpdate:
    ...
    func isLayoutBiggerThanVisibleBounds(at state: MessagesCollectionLayoutModelState,
    ...
    private func allAttributes(
        ...
        if let visibleRect {
            ...
            // max of 200 elements on the screen
    ...
    private func isLastItemInSection(_ itemPath: ItemPath, at state: MessagesCollectionLayoutModelState) -> Bool {
        let layout = layout(at: state)
